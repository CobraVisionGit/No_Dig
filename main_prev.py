...
# import cupy as cp
import torch
from Detector_Prev import Detector
import threading
...
toggle_imu = False
toggle_isp = True
confidence_threshold = 200 # Max 255. Luxonis recommends 200. I have seen 80 removes most noise
...
config.postProcessing.thresholdFilter.maxRange = 7000 # Default 200000
...
with dai.Device(pipeline, maxUsbSpeed=dai.UsbSpeed.HIGH) as device:
    ...
    # # ===============================
    # should_run = True
    # model_setup_complete = threading.Event()
    #
    # def display_images(qs, sync):
    #     global should_run
    #     cv2.namedWindow('temp_window', cv2.WND_PROP_FULLSCREEN)
    #     cv2.setWindowProperty('temp_window', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    #     while_index = 0
    #     while not model_setup_complete.is_set():
    #     # while while_index < 50:
    #         for q in qs:
    #             new_msg = q.tryGet()
    #             if new_msg is not None:
    #                 msgs = sync.add_msg(q.getName(), new_msg)
    #                 if msgs:
    #                     color = get_color_frame(msgs)
    #                     cv2.putText(color, "Loading Model!", (10 + while_index * 1, 10 + while_index * 1),
    #                                 cv2.FONT_HERSHEY_SIMPLEX, 1, (200, 0, 255), 2)
    #                     cv2.imshow("temp_window", color)
    #                     key = cv2.waitKey(1)
    #                     while_index += 1
    #
    #
    # def get_color_frame(msgs):
    #     if COLOR:
    ...
    #
    #
    # thread = threading.Thread(target=display_images, args=(qs, sync))
    # thread.start()
    # # ===============================
    ...
    # model_setup_complete.set()  # Signal that model setup is complete
    # should_run = False
        ...
        for q in qs:
            ...
            if new_msg is not None:
                ...
                if msgs:
                    ...
                    if while_index % 50 == 0:
                        # cv2.imwrite(f"Images/{save_index}.jpg", color)

                        # cv2.imwrite(f"Images/{save_index}_left.jpg", mono_left)
                        # cv2.imwrite(f"Images/{save_index}_right.jpg", mono_right)
                        save_index += 1
                        pass
                    ...
                    cv2.putText(output_color, f"Timestamp: {str(color_frame_data_timestamp)[:-3]}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
