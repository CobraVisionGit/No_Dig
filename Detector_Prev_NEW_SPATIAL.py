...
import math
from Spatial_Calculator import HostSpatialsCalc
...
class Detector:
    def __init__(self, device):
        self.host_spatials = HostSpatialsCalc(device)
        delta = 5
        self.host_spatials.setDeltaRoi(delta)
        self.DELTA = 5
    ...
    def to_image_old(self, X_cam, Y_cam, Z_cam):
    ...
    def to_image(self, X_cam, Y_cam, Z_cam):
        HFOV_rad = self.HFOV
        HFOV_rad = torch.tensor(HFOV_rad, device='cuda')

        angle_x = torch.atan(X_cam / Z_cam)
        angle_y = -torch.atan(Y_cam / Z_cam)

        midW = int(self.depth_frame.shape[1] / 2)
        midH = int(self.depth_frame.shape[0] / 2)

        x = midW + (angle_x * (midW / torch.tan(HFOV_rad / 2.0)))
        y = midH + (angle_y * (midW / torch.tan(HFOV_rad / 2.0)))
    ...
    def get_aruco_matrix(self, frame, depth_data, depth_frame_t=None):
        gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    ...
    #             spatials, centroid = self.host_spatials.calc_spatials(depth_data, (int(x), int(y)) )
    #             corner_3d_tensor = torch.tensor([spatials['x'], spatials['y'], spatials['z']], device=self.device, dtype=torch.float32)
    ...
    #             spatials, centroid = self.host_spatials.calc_spatials(depth_data, (int(x), int(y)) )
    #             midpoint_3d_tensor = torch.tensor([spatials['x'], spatials['y'], spatials['z']], device=self.device, dtype=torch.float32)
    ...
    #         spatials, centroid = self.host_spatials.calc_spatials(depth_data, (int(center_x), int(center_y)) )
    #         midpoint_3d_tensor = torch.tensor([spatials['x'], spatials['y'], spatials['z']], device=self.device, dtype=torch.float32)
    #         if midpoint_3d_tensor is None:
    #             self.marker_corners, self.marker_ids = None, None
    #             return
    ...
    #
    #         for i in range(len(self.aruco_rvecs)):
    #             cv2.drawFrameAxes(frame, self.intrinsics_rgb, self.dist_coeffs_rgb, self.aruco_rvecs[i], self.aruco_tvecs[i], 50 * self.marker_size)

    def annotate_results(self, frame, coordinates, class_indexes, depth_frame, depth_data):
        ...
        self.HFOV = self.host_spatials.get_hfov(depth_data)

        # Convert data to PyTorch tensors and move to GPU
        depth_frame_t = torch.from_numpy(depth_frame.astype(np.float32)).to(self.device)

        self.get_aruco_matrix(frame, depth_data, depth_frame_t=depth_frame_t)
        ...
        frame, transformation_matrix = self.draw_no_dig(frame, depth_frame, depth_data, coords_T_list, center_xy_TL_list,
    ...
    def get_info_from_vehicle_coords(self, center_xy_bucket_list, depth_frame_t):
        ...
        for center_xy_bucket in center_xy_bucket_list:
            ...
            spatials, centroid = self.host_spatials.calc_spatials(self.depth_data, (int(center_xy_bucket[0]), int(center_xy_bucket[1])))
            coords_spatial_center_bottom_vehicle = torch.tensor([spatials['x'], spatials['y'], spatials['z']], device=self.device, dtype=torch.float32)
    ...
    def get_info_from_coords(self, center_xy_TL, center_xy_TR, center_xy_TB, depth_frame_t):
        ...
        spatials, centroid = self.host_spatials.calc_spatials(self.depth_data, (int(center_xy_TL[0]), int(center_xy_TL[1])))
        coords_TL = torch.tensor([spatials['x'], spatials['y'], spatials['z']], device=self.device, dtype=torch.float32)
        spatials, centroid = self.host_spatials.calc_spatials(self.depth_data, (int(center_xy_TR[0]), int(center_xy_TR[1])))
        coords_TR = torch.tensor([spatials['x'], spatials['y'], spatials['z']], device=self.device, dtype=torch.float32)
        spatials, centroid = self.host_spatials.calc_spatials(self.depth_data, (int(center_xy_TB[0]), int(center_xy_TB[1])))
        coords_TB = torch.tensor([spatials['x'], spatials['y'], spatials['z']], device=self.device, dtype=torch.float32)
        ...
        spatials, centroid = self.host_spatials.calc_spatials(self.depth_data, (int(average_xy_TLTR[0]), int(average_xy_TLTR[1])))
        center_spatial_TL_TB = torch.tensor([spatials['x'], spatials['y'], spatials['z']], device=self.device, dtype=torch.float32)
    ...
    def get_normal_vector_from_coords(self, center_xy_TL, center_xy_TR, center_xy_TB, depth_frame_t):
        ...
        # Assuming center_xy_TL and center_xy_TR are torch tensors
        average_xy_TLTR = (center_xy_TL + center_xy_TR) / 2
    ...
    def _calc_angle(self, frame, offset, HFOV):
        HFOV_tensor = torch.tensor(HFOV / 2.0, device=offset.device, dtype=offset.dtype)
        return torch.atan(torch.tan(HFOV_tensor) * offset / (frame.shape[1] / 2.0))

    def _check_input(self, roi, frame):  # Check if input is ROI or point. If point, convert it to ROI
        self.DELTA = 5  # Take 10x10 depth pixels around point for depth averaging
        x = torch.clamp(roi[0], min=self.DELTA, max=frame.shape[1] - self.DELTA)
        y = torch.clamp(roi[1], min=self.DELTA, max=frame.shape[0] - self.DELTA)
    ...
    def draw_no_dig(self, frame, depth_frame, depth_data, coords_T_list, center_xy_TL_list, center_xy_TR_list, center_xy_TB_list,
                    center_xy_bucket_list):
        ...
        # if self.marker_ids is not None and len(self.marker_ids) > 0:
            ...
            for idx, marker_id in enumerate(self.marker_ids):
                ...
                if marker_id == 227:
                    ...
                    spatials, centroid = self.host_spatials.calc_spatials(self.depth_data, (int(marker_center[0]), int(marker_center[1])))
                    center_spatial_marker = torch.tensor([spatials['x'], spatials['y'], spatials['z']],device=self.device)
                            ...
                            if saved_zone_coords_TL_list is not None:
                                ...
                                if (len(saved_zone_center_spatial_list) + len(coords_T_list_temp) >= 3) and len(coords_T_list_temp) < 3 and not should_draw_from_T:
                                    ...
                                    for saved_zone_index_to_draw_index, saved_zone_index_to_draw in enumerate(saved_zone_indexes_to_draw):
                                        ...
                                        for center_xy in center_xy_to_use_list:
                                            ...
                                            spatials, centroid = self.host_spatials.calc_spatials(self.depth_data, (int(center_xy[0]), int(center_xy[1])))
                                            center_spatial_T = torch.tensor([spatials['x'], spatials['y'], spatials['z']], device=self.device, dtype=torch.float32)
                                ...
                                if not should_draw_from_T:
                                    for saved_zone_index_to_draw_index, saved_zone_index_to_draw in enumerate(saved_zone_indexes_to_draw):
                                        ...
                                        center_spatial_TL_TB_memory = (delta_rmat @ saved_zone_offset_vector) + center_spatial_marker
                                        center_spatial_TL_TB_memory = center_spatial_TL_TB_memory.float()
                                    ...
                                    for saved_zone_center_spatial_index, saved_zone_center_spatial in enumerate(saved_zone_center_spatial_list):
                                        ...
                                        coords_TL_memory = delta_rmat @ (coords_TL_memory - saved_zone_marker_center) + center_spatial_marker
                                        center_xy_TL_memory = self.to_image(*coords_TL_memory)
                                        coords_TR_memory = delta_rmat @ (coords_TR_memory - saved_zone_marker_center) + center_spatial_marker
                                        center_xy_TR_memory = self.to_image(*coords_TR_memory)
                                        coords_TB_memory = delta_rmat @ (coords_TB_memory - saved_zone_marker_center) + center_spatial_marker
                                        center_xy_TB_memory = self.to_image(*coords_TB_memory)
